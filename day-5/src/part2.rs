use std::{collections::HashMap, iter::zip, fmt::Error, ops::Range};
use indicatif::ProgressIterator;
use rayon::prelude::*;
use itertools::Itertools;
use nom::{IResult, bytes::complete::{tag, take_till}, multi::{separated_list0, many1}, character::{complete::{space1, u64, multispace0, line_ending}, is_digit}, Err, sequence::{separated_pair}};
...
fn create_line(destination: u64, source: u64, len: u64) -> Map{
	Map {dest: destination, source, len}
}

fn parse_seeds(input:&str) -> IResult<&str, Vec<Range<u64>>>{
	let (input, _) = tag("seeds: ")(input)?;
	let (input, list) = separated_list0(space1, separated_pair(u64, space1, u64))(input)?;
    let result = list.iter().map(|(i, j)| *i..*i+*j).collect_vec();
    Ok((input, result))
}
...
fn parse_map(input:&str) -> IResult<&str, Maps> {
	...
	let lines = lines.into_iter().filter(|v| !v.is_empty()).collect::<Vec<Vec<u64>>>();
...
fn get_location(maps: &Vec<Maps>, seed: u64) -> u64{
	let mut prev = seed;
	for map in maps {
		prev = map.get(prev)
	}
...
pub fn process(
    ...
    let location = seeds.iter().flat_map(|i| i.clone().into_iter()).map(|s| get_location(&maps, s)).min().unwrap();
...
";	
	...
	fn map_creation() {
		...
		assert_eq!(m.get(98), 50_u64);
		assert_eq!(m.get(97), 99_u64);
		assert!(m.get(48) == 48)
	}
